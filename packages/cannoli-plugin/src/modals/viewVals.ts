...
export class ValTownModal extends Modal {
	...
	onOpen() {
		...
		this.cannoliFunctions.forEach((func) => {
			...
			const openCanvasButton = canvasCell.createEl("button", {
				text: "Canvas",
			});
			...
			const copyButton = copyUrlCell.createEl("button", {
				text: "📋 URL",
			});
			copyButton.addEventListener("click", () => {
				navigator.clipboard
					.writeText(func.httpEndpointUrl)
					.then(() => {
			...
			const copyCurlButton = copyCurlCell.createEl("button", {
				text: "📋 cURL",
			});
...
${Object.keys(func.cannoliFunctionInfo.params).length > 0 ? `-d '${JSON.stringify(Object.fromEntries(Object.keys(func.cannoliFunctionInfo.params).map((param) => [param, "value"])), null, 2)}'` : ""}`;
			...
			if (!func.localExists) {
				const syncButton = syncStatusCell.createEl("button", {
					text: "Download",
				});
				syncButton.addEventListener("click", async () => {
					this.createCanvas(
						func.cannoliFunctionInfo.canvasName,
						JSON.stringify(func.cannoliFunctionInfo.cannoli),
					);
					new Notice(
						`Copied ${func.cannoliFunctionInfo.canvasName} to vault`,
					);
					const newCannoliFunctions =
						await this.getCannoliFunctions();
					const newModal = new ValTownModal(
						this.app,
						newCannoliFunctions,
						this.getCannoliFunctions,
						this.openCanvas,
						this.valtownApiKey,
						this.bakeToValTown,
						this.createCanvas,
