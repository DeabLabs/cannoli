...
export class CannoliGroup extends CannoliVertex {
  ...
  constructor(
    ...
    this.members = getGroupMembersFromData(groupData.id, fullCanvasData);
  ...
  getMembers(): CannoliVertex[] {
    return this.members.map((member) => this.graph[member] as CannoliVertex);
  }
  ...
  noEdgeDependenciesRejected(): boolean {
    ...
    for (const dependency of this.dependencies) {
      ...
      if (this.graph[dependency].kind === "edge") {
        if (this.graph[dependency].status === CannoliObjectStatus.Rejected) {
          return false;
        }
      }
    }
  ...
  checkOverlap(): void {
    ...
    for (const objectKey in this.graph) {
      ...
      if (object instanceof CannoliVertex) {
        ...
        const objectData = this.canvasData.nodes.find(
  ...
  validate() {
    ...
    if (this.fromForEach) {
      const { crossingInEdges, crossingOutEdges } =
        this.getCrossingAndInternalEdges();
      ...
      if (crossingInItemEdges.length > 0) {
        this.error(
          `List edges can't cross into parallel groups. Try putting the node it's coming from inside the parallel group or using a non-list edge and an intermediary node.`,
        );
        return;
      }
      ...
      const crossingOutItemOrListEdges = crossingOutEdges.filter(
        (edge) =>
          this.graph[edge.id].type === EdgeType.Item ||
      ...
      if (crossingOutItemOrListEdges.length > 0) {
        for (const edge of crossingOutItemOrListEdges) {
          ...
          const crossingInGroups = edgeObject.crossingInGroups.map(
            (group) => this.graph[group] as CannoliGroup,
          );

          const crossingInParallelGroups = crossingInGroups.filter(
            (group) => group.fromForEach,
          );
      ...
      const itemOrListEdges = this.incomingEdges.filter(
        (edge) =>
          this.graph[edge].type === EdgeType.Item ||
          this.graph[edge].type === EdgeType.List,
      );
      if (itemOrListEdges.length < 1) {
        this.error(
          `Parallel groups must have at least one incoming list arrow (cyan, labeled).`,
        );
        return;
      } else if (itemOrListEdges.length > 1) {
        // Check if one of the edges crosses a fromForEach group
        const itemEdges = itemOrListEdges.filter((edge) =>
          (this.graph[edge] as CannoliEdge).crossingOutGroups.some(
            (group) => (this.graph[group] as CannoliGroup).fromForEach,
          ),
        );
        ...
        this.error(
          `Parallel groups can't have more than one incoming list arrow.`,
        );
        return;
      }
    }
