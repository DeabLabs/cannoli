...
export class ReferenceNode extends ContentNode {
      ...
      const filteredVariableValueEdges = variableValueEdges.filter(
        (variableValueEdge) => {
          return (
            ...
            variableValueEdge.edgeModifier !== EdgeModifier.Folder &&
            variableValueEdge.edgeModifier !== EdgeModifier.Property
          );
        },
      );

      // Then, filter the variable values by the filtered edges
      const filteredVariableValues = variableValues.filter((variableValue) => {
        return filteredVariableValueEdges.some((filteredVariableValueEdge) => {
  ...
  async getContent(): Promise<string> {
    ...
    if (this.reference) {
      if (this.reference.type === ReferenceType.Note) {
        ...
        const content = this.getContentFromFloatingNode(this.reference.name);
        if (content !== null) {
          return content;
        } else {
          this.error(
      ...
      } else if (this.reference.type === ReferenceType.CreateNote) {
  ...
  async editProperty(propertyName: string, newContent: string): Promise<void> {
    if (this.run.isMock) {
      return;
    }
  ...
  async loadOutgoingEdges(
    ...
    for (const edge of this.outgoingEdges) {
      ...
      if (edgeObject.edgeModifier === EdgeModifier.Property) {
        ...
        if (
          !this.reference.name.startsWith("[[") &&
          !this.reference.name.endsWith("]]")
        ) {
