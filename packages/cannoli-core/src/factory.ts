...
export class CannoliFactory {
  ...
  createNodeData(
    node: CannoliCanvasFileData | CannoliCanvasLinkData | CannoliCanvasTextData,
  ...
  createEdgeData(edge: CannoliCanvasEdgeData): CannoliEdgeData | null {
    ...
    addMessages =
      labelInfo?.addMessages !== undefined && labelInfo?.addMessages !== null
  ...
  createForEachDuplicates(
    ...
    const groups = data.nodes
      ...
      .sort(
        (a, b) => b.cannoliData.groups.length - a.cannoliData.groups.length,
      );
  ...
  createDuplicateGroup(
    ...
    duplicateInternalEdges.forEach((edge) => {
      edge.fromNode = `${edge.fromNode}-${index}`;
      edge.toNode = `${edge.toNode}-${index}`;
      edge.cannoliData.crossingInGroups = edge.cannoliData.crossingInGroups.map(
      ...
      for (const subGroup of subGroups) {
        if (member.cannoliData.groups.includes(subGroup.id)) {
          member.cannoliData.groups = member.cannoliData.groups.map(
            (groupId) =>
              groupId === subGroup.id ? `${groupId}-${index}` : groupId,
          );
        }
      }
      // replace the original group id with the duplicate group id
      member.cannoliData.groups = member.cannoliData.groups.map((groupId) =>
        groupId === originalGroup.id ? `${groupId}-${index}` : groupId,
      );
    });
  ...
  getCallNodeType(vertex: CannoliCanvasTextData): CallNodeType | null {
    ...
    if (
      outgoingEdges.some((edge) => this.getEdgeType(edge) === EdgeType.Field)
    ) {
      return CallNodeType.Form;
    }
    // If it has any outgoing choice edges, it's a choose node
    else if (
      outgoingEdges.some((edge) => this.getEdgeType(edge) === EdgeType.Choice)
    ) {
  ...
  getEdgeType(edge: CannoliCanvasEdgeData): EdgeType | null {
    if (edge.color) {
      ...
      if (this.edgeColorMap[edge.color]) {
        ...
        // If the type from the color map is chatconverter, return the subtype
        else if (this.edgeColorMap[edge.color] === EdgeType.ChatConverter) {
          ...
          if (
            sourceNode &&
            this.getNodeIndicatedType(sourceNode) === IndicatedNodeType.Call &&
            targetNode &&
            this.getNodeIndicatedType(targetNode) === IndicatedNodeType.Content
          ) {
      ...
      if (this.edgePrefixMap[edge.label[0]]) {
        ...
        // If the type from the color map is config
        else if (this.edgePrefixMap[edge.label[0]] === EdgeType.Config) {
      ...
      if (!sourceNode || !targetNode) {
        throw new Error(`Edge: ${edge.label} source or target not found`);
      }
