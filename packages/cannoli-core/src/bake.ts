...
export async function bake({
	...
	if (stoppage.reason == "error") {
		return new Error(
			"There's an error in the cannoli. Please fix it before baking.",
		);
	}
	...
	const llmConfigsWithBaseURLorAPIKey = llmConfigs?.filter(
	...
	const llmConfigsWithReferencedProvider =
		llmConfigsWithBaseURLorAPIKey?.filter(
			(config, index) =>
				stoppage.providersReferenced.names.includes(config.provider) ||
				index === 0,
		);

	// Filter out actions without importInfo
	const actionsWithImportInfo = actions?.filter(
		(action) => action.importInfo,
	);

	const actionsOrHttpTemplatesReferenced =
		stoppage.actionsOrHttpTemplatesReferenced;

	// Filter out actions and http templates that are not referenced
	const referencedActions = actionsWithImportInfo?.filter((action) =>
		actionsOrHttpTemplatesReferenced.names.includes(action.name),
	);
	const referencedHttpTemplates = httpTemplates?.filter((template) =>
		actionsOrHttpTemplatesReferenced.names.includes(template.name),
	);

	const secretsReferenced: string[] = [];

	// Check each secret
	outerLoop: for (const [key] of Object.entries(secrets || {})) {
		...
		for (const template of referencedHttpTemplates || []) {
			...
			if (
				...
				template.bodyTemplate.includes(`{{${key}}}`)
			) {
				secretsReferenced.push(key);
				continue outerLoop;
			}
	...
	const filteredSecrets = Object.fromEntries(
		Object.entries(secrets || {}).filter(([key]) =>
			secretsReferenced.includes(key),
		),
	);
...
export function writeCode({
	...
	const importTemplate = generateImportTemplates(
		cannoliInfo,
		language,
		runtime,
		actions,
	...
	const optionalArgTemplates =
		`${secretTemplate}${configTemplate}${actionsTemplate}${httpTemplatesTemplate}`.trim();

	const argDeclarations = `${optionalArgTemplates}${llmConfigTemplate}
const cannoli = ${JSON.stringify(cannoli, null, 2)};`;

	const generatedFunction = generateFunction(
		cannoliInfo,
		language,
		argDeclarations,
		availableArgs,
...
export function parseCannoliFunctionInfo(
	fileContent: string,
): CannoliFunctionInfo | null {
	...
	for (const line of lines) {
		...
		if (paramTypeMatch) {
			paramType =
				CannoliParamType[
					paramTypeMatch[1] as keyof typeof CannoliParamType
				] || CannoliParamType.Object;
		}
		...
		if (returnTypeMatch) {
			returnType =
				CannoliReturnType[
					returnTypeMatch[1] as keyof typeof CannoliReturnType
				] || CannoliReturnType.Object;
		}
	}
...
function extractCannoliJson(fileContent: string): unknown {
	...
	const cannoliJson = fileContent.slice(
		cannoliStart + "const cannoli = ".length,
		cannoliEnd + 1,
	);
	try {
...
export async function callCannoliFunction(
	...
	switch (paramType) {
		case CannoliParamType.String:
			if (typeof params !== "string") {
				throw new Error(
					`Expected a single string parameter for function ${cannoliInfo.name}.`,
				);
			}
			break;
		...
		case CannoliParamType.Array:
			if (!Array.isArray(params)) {
				throw new Error(
					`Expected an array of parameters for function ${cannoliInfo.name}.`,
				);
			}
			break;
		...
		case CannoliParamType.Object: {
			if (typeof params !== "object" || params === null) {
				throw new Error(
					`Expected an object of parameters for function ${cannoliInfo.name}.`,
				);
			}
			...
			if (missingParams.length > 0) {
				throw new Error(
					`Missing required parameters: ${missingParams.join(", ")} for function ${cannoliInfo.name}.`,
				);
			}
			break;
		}
		case CannoliParamType.Void:
			if (params !== undefined) {
				throw new Error(
					`Expected no parameters for function ${cannoliInfo.name}.`,
				);
			}
			break;
		...
		default:
			throw new Error(
				`Unknown parameter type for function ${cannoliInfo.name}.`,
			);
	}
	...
	if (returnType === CannoliReturnType.Void) {
		...
		throw new Error(
			`Unknown return type for function ${cannoliInfo.name}.`,
		);
	}
}
		...
		switch (paramType) {
			...
			case CannoliParamType.Object:
				return requiredParams
					.map(
						(param, index) =>
							`- \`${param}\` (string)${paramDescriptions[index] ? ` - ${paramDescriptions[index]}` : ""}`,
					)
					.join("\n");
			case CannoliParamType.Void:
			default:
...
function generateMetadata(cannoliInfo: CannoliFunctionInfo): string {
	...
	const params =
		cannoliInfo.params && Object.keys(cannoliInfo.params).length > 0
	...
	const returns =
		cannoliInfo.returns && Object.keys(cannoliInfo.returns).length > 0
...
function generateCommentBlock(
	...
	const formatDescription = (desc?: string) =>
		desc
			? desc
					...
					.join("\n")
			: "";
	const capitalizedCannoliName =
		cannoliInfo.name.charAt(0).toUpperCase() + cannoliInfo.name.slice(1);
	...
	switch (cannoliInfo.paramType) {
		...
		case CannoliParamType.Array:
		case CannoliParamType.String:
			for (const [arg, description] of Object.entries(
				cannoliInfo.params,
			)) {
	...
	switch (cannoliInfo.returnType) {
		...
		case CannoliReturnType.String: {
			const [result, description] = Object.entries(
				cannoliInfo.returns,
			)[0];
...
function generateTypeDefinitions(
	...
	const resultType =
		Object.keys(cannoliInfo.returns)
	...
	if (language === "typescript") {
		if (
			cannoliInfo.paramType === CannoliParamType.Object &&
			args.length > 0
		) {
		...
		if (
			cannoliInfo.returnType === CannoliReturnType.Object &&
			Object.keys(cannoliInfo.returns).length > 0
		) {
		...
		if (
			...
			resultTypeDef = `/**\n * @typedef {Object} ${cannoliInfo.name.charAt(0).toUpperCase() + cannoliInfo.name.slice(1)}Return\n${Object.keys(
				...
				.map((result) => ` * @property {string} ${result}`)
				.join("\n")}\n */\n\n`;
		}
	}
	...
	switch (paramType) {
		case CannoliParamType.String:
			argsString = args
				.map(
					(arg) =>
						`${arg}${language === "typescript" ? ": string" : ""}`,
				)
				.join(", ");
			break;
		case CannoliParamType.Array:
...
function generateRunFunctionCall(
	cannoliInfo: CannoliFunctionInfo,
...
function generateFunction(
	...
	const runFunctionCall = generateRunFunctionCall(
		cannoliInfo,
		availableArgs,
		language,
		includeTypes,
...
function generateImportTemplates(
	...
	if (actions) {
		actions.forEach((action) => {
			let importPath =
				action.importInfo?.path ||
				"Paste import path or implement action";
			...
			importMap[importPath].push(
				action.importInfo?.name || "UnknownAction",
			);
		});
	}

	// Build the import statements
	const importStatements = Object.entries(importMap)
		.map(([path, imports]) => {
			if (language === "typescript") {
				if (imports.length > 4) {
					return `import {\n  ${imports.join(",\n  ")}\n} from "${path}";\n`;
...
function generateSecretTemplate(
	...
	const secretLines = Object.keys(secrets)
		.map((key) => {
			switch (runtime) {
				case "node":
	...
	const cleanedLines = lines.map((line) => {
		...
		if (changeIndentToFour) {
			...
			if (match) {
				...
				if (spaces % 2 === 0) {
					cleanedLine =
						" ".repeat(spaces * 2) + cleanedLine.trimStart();
				}
			}
		}
