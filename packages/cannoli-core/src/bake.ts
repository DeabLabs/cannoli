...
export async function bake({
  ...
  if (dynamicParamAndReturnTypes) {
    ...
    paramType =
      argNames.length > 0 ? CannoliParamType.Object : CannoliParamType.Void;
    returnType =
  ...
  const cannoliInfo: CannoliFunctionInfo = {
    ...
    params: Object.fromEntries(argNames.map((name) => [name, ""])) as Record<
      string,
      string
    >,
...
export function parseCannoliFunctionInfo(
  ...
  for (const line of lines) {
    ...
    if (paramTypeMatch) {
      paramType =
        CannoliParamType[paramTypeMatch[1] as keyof typeof CannoliParamType] ||
        CannoliParamType.Object;
    }
...
function generateFunctionSignature(
  ...
  switch (paramType) {
    case CannoliParamType.String:
      argsString = args
        .map((arg) => `${arg}${language === "typescript" ? ": string" : ""}`)
        .join(", ");
      break;
    case CannoliParamType.Array:
...
function generateImportTemplates(
  ...
  if (actions) {
    actions.forEach((action) => {
      let importPath =
        action.importInfo?.path || "Paste import path or implement action";
