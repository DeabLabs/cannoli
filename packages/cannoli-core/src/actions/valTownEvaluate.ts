...
export const valTownEvaluate: Action = {
  ...
  function: async (_args) => {
    // this is very unsafe
    const { code, VALTOWN_API_KEY, fetcher } = _args as {
      code: string;
  ...
  argInfo: {
    code: {
      category: "arg",
      prompt:
        "Ensure your code is valid JavaScript or TypeScript which directly returns a result by using immediately invoked function expressions or top-level return statements.",
    },
    args: {
      category: "extra",
